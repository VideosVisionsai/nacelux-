...
def run_import(extra):
    env = {k: v for k, v in os.environ.items() if k not in {
        'NACELUX_ENV', 'DB_PROVIDER', 'DATABASE_URL', 'DB_SSLMODE', 'MIGRATION_DATABASE_URL', 'SUPABASE_URL',
...
'''
    ...
    def test_production_storage_never_defaults_to_local(self):
        result = subprocess.run(
            [sys.executable, '-c', 'import sys; sys.path.insert(0, "backend"); import document_storage'],
            cwd=ROOT,
            env={**os.environ, 'NACELUX_ENV': 'production', 'DOCUMENT_STORAGE_PROVIDER': 'local'},
            capture_output=True, text=True, timeout=10,
        )
        self.assertNotEqual(result.returncode, 0)
        self.assertTrue('DOCUMENT_STORAGE_PROVIDER' in result.stderr or 'Supabase Storage' in result.stderr)
