...
class PostgreSQLIntegrationTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        role=cls.conn.execute('SELECT rolbypassrls,rolsuper FROM pg_roles WHERE rolname=current_user').fetchone()
        if not role or role['rolbypassrls'] or role['rolsuper']:
            raise AssertionError('PostgreSQL integration role bypasses RLS or is superuser')
    ...
    def _insert_tenant_rows(self, org, user, company, job):
        ...
        self.conn.execute(
            """INSERT INTO jobs(id,organization_id,job_type,status,started_at,finished_at,records_processed,error,payload,attempt,schedule)
    ...
    def test_jobs_use_explicit_columns_and_lifecycle(self):
        ...
        self.conn.execute("UPDATE jobs SET status='RETRY',schedule='RETRY',started_at=CURRENT_TIMESTAMP WHERE id=%s", (self.job_a,))
        self.conn.execute("UPDATE jobs SET status='RUNNING',attempt=attempt+1,schedule=NULL WHERE id=%s AND status='RETRY'", (self.job_a,))
