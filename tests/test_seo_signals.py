...
class BusinessSignalsTests(unittest.TestCase):
    def setUp(self):
        ...
        with connect() as db:
            db.executescript(database.SCHEMA);db.execute("INSERT INTO organizations VALUES(?,?,?,?)",('org','Org','org','2026-01-01'));db.execute("INSERT INTO companies(id,organization_id,company_name,creation_date,website_status,google_status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)",('c','org','New Company',date.today().isoformat(),'NOT_FOUND','NOT_FOUND','2026-01-01','2026-01-01'));db.execute("INSERT INTO website_discovery_runs(id,organization_id,company_id,status,provider,query_text,started_at,candidates_found,selected_candidate_id) VALUES(?,?,?,?,?,?,?, ?,?)",('run','org','c','SUCCESS','test','q','2026-01-01',0,None));db.execute("INSERT INTO seo_audits(id,organization_id,company_id,status,seo_score,findings) VALUES(?,?,?,?,?,?)",('s','org','c','SUCCESS',20,'[]'));db.execute("INSERT INTO digital_checks(id,organization_id,company_id,channel,status,confidence,checked_at,details) VALUES(?,?,?,?,?,?,?,?)",('dw','org','c','Website','NOT_FOUND',1,'2026-01-01','{}'));db.execute("INSERT INTO digital_checks(id,organization_id,company_id,channel,status,confidence,checked_at,details,source_provider) VALUES(?,?,?,?,?,?,?,?,?)",('dg','org','c','Google Business','NOT_FOUND',1,'2026-01-01','{}','google_places'))
